...
getMinEventCount <- function(anchorKeyword="anchor stim", basedir){
   ...
   for(ananchor in anchors_list){
      ...
      thisFCSobject <- read.FCS(ananchor, transformation=NULL, which.lines=whichlines, 
                                truncate_max_range = FALSE, emptyValue = getOption("flowCore.read.FCS.emptyValue"))
...
fc_read <- function(fname, trans=TRUE, which.lines=NULL, fullnames=TRUE){
   fc <- read.FCS(fname, transformation=NULL, which.lines=which.lines, 
                  truncate_max_range = FALSE, emptyValue = getOption("flowCore.read.FCS.emptyValue")) # flowFrame
   fce <- exprs(fc); # matrix
...
options(flowCore.read.FCS.emptyValue = TRUE)
getOption("flowCore.read.FCS.emptyValue")
